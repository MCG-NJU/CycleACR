...
class TemporalCrop(object):
    def __init__(self, frame_num, sample_rate, temporal_jitter=False):
    ...
    def __call__(self, clip, target, transform_randoms):
        ...
        if self.temporal_jitter:
            ...
            temporal_offset = random.randint(0, self.sample_rate - 1)
        else:
            # if no jitter, select the middle
            temporal_offset = (self.sample_rate - 1) // 2

        # determine frame start based on frame_num, sample_rate and the jitter shift.
        frame_start = (raw_frame_num - self.frame_num * self.sample_rate) // 2 + temporal_offset
...
class Resize(object):
    def __init__(self, min_size, max_size, use_clip=False):
...
class Identity(object):
    ...
    def __call__(self, clip, target, tranform_randoms):
        return clip, target, tranform_randoms
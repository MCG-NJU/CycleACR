...
class Checkpointer(object):
    ...
    def _load_model(self, checkpoint, no_head):
        ckpt = checkpoint.pop("model")
        if "module" in ckpt.keys():
            load_state_dict(self.model, ckpt.pop("module"), no_head)
        elif "state_dict" in ckpt.keys():
            load_state_dict(self.model, ckpt.pop("state_dict"), no_head)
        else:
            load_state_dict(self.model, ckpt, no_head)

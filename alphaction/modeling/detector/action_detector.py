...
class ActionDetector(nn.Module):
    ...
    def forward(self, slow_video, fast_video, boxes, extras={}, part_forward=-1):
        # part_forward is used to split this model into two parts.
        # if part_forward<0, just use it as a single model.
        # if part_forward=0, extract pooled feature and conduct cycle modeling.
        # if part_forward=1, conduct instance interaction (w/ context-aware memory bank) and give final result.
        # implemented in roi_heads

        slow_features = fast_features = None
        if part_forward != 1:
            if self.backbone.num_pathways == 2:
                slow_features, fast_features = self.backbone(slow_video, fast_video)
            else:
                slow_features = self.backbone(slow_video)

        result, detector_losses, loss_weight, detector_metrics = self.roi_heads(slow_features, fast_features, boxes, extras, part_forward)

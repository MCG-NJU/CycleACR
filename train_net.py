...
def train(cfg, local_rank, distributed, tblogger=None, transfer_weight=False, adjust_lr=False, skip_val=False,
    ...
    logger = logging.getLogger("alphaction.model")
    logger.info(model)
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info('number of params: {}'.format(str(n_parameters)))
    ...
    val_after = cfg.SOLVER.EVAL_AFTER
    val_period = cfg.SOLVER.EVAL_PERIOD

    mem_active = cfg.MODEL.CYCLE_ACR.MEM_ACTIVE
    ...
    do_train(
        ...
        distributed,
        mem_active,
        num_val_dataset=len(cfg.DATASETS.TEST),
        output_dir=cfg.OUTPUT_DIR,
    )

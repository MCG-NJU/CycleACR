...
from alphaction.modeling.roi_heads.roi_heads_3d import build_3d_roi_heads
from config_dataclass import Config as ROIActionHeadCfg
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        head_cfg = ROIActionHeadCfg()
        self.roi_heads = build_3d_roi_heads(head_cfg, self.num_features)
    ...
    def forward(self, x, boxes, extras={}, part_forward=-1):
        x_features = None
        if part_forward != 1:
            ...
            x = self.norm(x)
            x_features = x.reshape(B, t, h, w, -1).permute(0, 4, 1, 2, 3)  # [b c t h w]

            # x_features = x_features.float()

        outputs, result = self.roi_heads(x_features, None, boxes, extras, part_forward)
